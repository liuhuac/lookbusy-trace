...
static int* cpu_trace_buffer; /*buffer storing cpu trace data*/
static int* mem_trace_buffer; /*buffer storing mem trace data*/
static int trace_length = 288; /*default trace length*/
static time_t trace_start_time; /*trace offset based on this timestamp*/
static int trace_time_inval = 5; /*default interval of trace 5 seconds*/
static size_t total_mem_amount;
static size_t base_mem_amount = 0; //438530048;
...
static int read_cpu_trace_file(const char *filename) {
	FILE *f;
	char s[8];
	f = fopen(filename, "r");
	int i=0;
	...
	cpu_trace_buffer = malloc(sizeof(int)*trace_length);
	if (cpu_trace_buffer == NULL) {
		perror("malloc cpu_trace_buffer failed");
		_exit(1);
	}

	while (fgets(s, sizeof(s)-1, f) != NULL) {
		s[sizeof(s)-1] = '\0';
		cpu_trace_buffer[i] = atoi(s);
		i++;
		if(i>=trace_length) {
			break;
		}
...
static int read_mem_trace_file(const char *filename) {
	FILE *f;
	char s[8];
	f = fopen(filename, "r");
	int i=0;
	...
	mem_trace_buffer = malloc(sizeof(int)*trace_length);
	if (mem_trace_buffer == NULL) {
		perror("malloc cpu_trace_buffer failed");
		_exit(1);
	}
...
static void cpu_spin_calibrate(int util, uint64_t *busycount, suseconds_t *sleeptime)
{
    ...
    say(2, "cpu_spin (%d): measuring CPU\n", getpid());
    ...
    *sleeptime = 100000 - *busycount / (counter / elapsed);
    ...
    say(2, "cpu_spin (%d): est. %d%% util at %"PRIu64" cycles, %u usec sleep\n",
                                    ...
                                    time_t curtime)
{
    ...
    if (mode == UTIL_MODE_TRACE) {
    	time_t current_time = time(NULL);
    	int index = (int)(current_time - trace_start_time) / trace_time_inval;
    	double level = (double)cpu_trace_buffer[index];/*Read from trace buffer*/

        /*make sure level scale to [0,100]*/
        if( level<0) {
            perror("Input trace value not in [0,100]\n");
            _exit(1);
        }
        if( level>100 ) level = 100;

    	say(2,"cpu_spin_compute_util (%d): index=%d, level=%f\n", getpid(), index, level);
    	return level;
    }
    /* shouldn't get here */
    return -1;
...
static size_t get_total_mem() {
	...
	if ((r = regexec(ex, s, 2, matches, 0)) != 0) {
		char errbuf[128];
		regerror(r, ex, errbuf, sizeof(errbuf)-1);
		errbuf[sizeof(errbuf)-1] = '\0';
	...
	total_mem = atol(s + matches[1].rm_so);

	say(2, "total_memory(kB): %lu", total_mem);

	fclose(f);

	return total_mem*1000 - base_mem_amount; /*kB to bytes*/
}

static size_t mem_stir_compute_util() {
	time_t current_time =  time(NULL);
	int index = (int)(current_time - trace_start_time) / trace_time_inval;
	double level = (double)mem_trace_buffer[index];/*Read from trace buffer*/

	/*make sure level scale to [0,1]*/
	level = level / 100;
	if( level<0) {
		perror("Input trace value not in [0,100]\n");
		_exit(1);
	...
	say(1, "mem_stir_compute_util (%d): total_mem_amount %lu bytes...\n", getpid(), total_mem_amount);
	size_t mem = (size_t)(total_mem_amount * level);
	say(1, "mem_stir_compute_util (%d): stirring %lu bytes...\n", getpid(), mem);
...
static void mem_stir(long long asz, long long dummy, long long dummy2, void *dummyp, void *dummyp2)
{
    ...
    if(c_cpu_util_mode == UTIL_MODE_TRACE) {
    	sz = mem_stir_compute_util();
    	trace_index = 0;
    }

    say(1, "mem_stir (%d): stirring %lu bytes...\n", getpid(), sz);
    if ((mem_stir_buffer = (char *)malloc(sz)) == NULL) {
        perror("malloc");
        _exit(1);
    }

    say(1, "mem_stir (%d): dirtying buffer...\n", getpid());
    for (p = mem_stir_buffer; p < mem_stir_buffer + sz; p++) {
        *p = (char)((uintptr_t)p & 0xff);
    }
    say(1, "mem_stir (%d): dirtying buffer done\n", getpid());
...
//        say(1, "before sleep, place=0");

        if(c_cpu_util_mode == UTIL_MODE_TRACE) {
        	current_time = time(NULL);
//        	say(1,"current_time=%ld\n",current_time);
//        	say(1,"start_time=%ld\n",trace_start_time);
//        	say(1,"current_time-start_time=%d\n",current_time - trace_start_time);
        	int new_index = (int)(current_time - trace_start_time) / trace_time_inval;
//        	say(1, "mem_stir (%d): new_index=%d\n", getpid(), new_index);

//        	say(1, "before sleep, place=1\n");

        	if(trace_index != new_index) {
//        		say(1, "mem_stir (%d): new_index=%d\n", getpid(), new_index);

//        		say(1, "before sleep, place=2\n");

        		sz = mem_stir_compute_util();
        		trace_index = new_index;
        		free(mem_stir_buffer);
...
//        		say(1, "before sleep, place=4\n");

        		sp = mem_stir_buffer;
        		dp = mem_stir_buffer;
        		say(1, "mem_stir (%d): done for index=%d\n", getpid(), trace_index);
        	}
        }
...
int main(int argc, char **argv)
{
    ...
    static const struct option long_options[] = {
        ...
        { "cpu-file", 1, NULL, 'C' },
        { "mem-file", 1, NULL, 'R' },
        { "trace-interval", 1, NULL, 'I' },
        { 0, 0, 0, 0 }
    };

    while ((c = getopt_long(argc, argv,
                            "n:b:c:d:D:f:m:M:p:P:r:qvVhuTC:R:I:B:",
                            long_options, NULL)) != -1) {
        switch (c) {
            default:
            case 'h': usage(); break;
            ...
            case 'c':
                ...
                break;
            case 'T':/*trace mode*/
            	c_cpu_util_mode = UTIL_MODE_TRACE;
            	break;
            case 'C':/*CPU trace file*/
            	if (!optarg || !*optarg)
            		break;
            	if (read_cpu_trace_file(optarg) < 0) {
            		err("Couldn't find: '%s'\n", optarg);
            		return 1;
            	}
            	break;
            case 'R':/*MEM trace file*/
            	if (!optarg || !*optarg)
            	            		break;
            	if (read_mem_trace_file(optarg) < 0) {
            		err("Couldn't find: '%s'\n", optarg);
            		return 1;
            	}
            	break;
            case 'I':/*trace time interval*/
            	if(!(trace_time_inval = atoi(optarg))){
            		err("Wrong input for -I: '%s'\n", optarg);
            		usage();
            		return 1;
            	}
            	break;
            case 'B':/*base memory amount*/
            	if(!(base_mem_amount = atoi(optarg))){
					err("Wrong input for -B: '%s'\n", optarg);
					usage();
					return 1;
            	}
            	break;
    ...
    if (c_cpu_util_mode == UTIL_MODE_TRACE) {/*initialize variables for trace workload*/
    	trace_start_time = time(NULL);
    	total_mem_amount = get_total_mem();
    }
    ...
    if (c_mem_util != 0 || c_cpu_util_mode == UTIL_MODE_TRACE) {
